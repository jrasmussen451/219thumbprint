four51.app.factory('ProductDisplayService', ['$sce', '$451', 'Variant', 'Product', function($sce, $451, Variant, Product){
    ...
    function calcTotal(lineItem){
        ...
        var totalLine = one * (priceBreak.Price + amountPerQty);

        totalLine += one * priceBreak.Price * (percentagePerLine / 100);
        totalLine += fixedAddPerLine;

        // console.log("Total Line: " + totalLine);
        lineItem.VarTotal = totalLine;
    }
    function productViewScope(scope){
        // console.log(scope.LineItem.Quantity);
